...
export const ensureSampleData = async (): Promise<boolean> => {
  try {
    // 기존 데이터 확인
    ...
    // 데이터가 없으면 샘플 데이터 추가
    if (count === 0) {
      const { error: insertError } = await supabase.from('memos').insert(sampleMemosData)

      if (insertError) {
        console.error('Failed to seed sample data:', insertError)
        return false
      }
...
export const resetToSampleData = async (): Promise<void> => {
  try {
    ...
    // 샘플 데이터 추가
    const { error } = await supabase.from('memos').insert(sampleMemosData)
